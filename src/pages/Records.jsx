...
async function handleExport() {
  ...
  <h1 className="text-base font-semibold text-gray-800">服務紀錄</h1>
  <div className="flex items-center gap-3">
    <p className="text-xs text-gray-400">根據簽到紀錄自動計算</p>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        <div className="card mb-4">
          <div className="flex gap-2 flex-wrap items-center">
            <select className="input w-36" value={filter.vol}
              onChange={e => setFilter(f => ({ ...f, vol: e.target.value }))}>
              <option value="all">所有志工</option>
              {volunteers.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
            </select>
            <select className="input w-28" value={filter.group}
            ...
            <select className="input w-24" value={filter.year}
            ...
            <select className="input w-24" value={filter.month}
            ...
            <tbody>
              {records.length === 0 ? (
                ...
                <tr key={r.id}>
                  ...
                  <td className="table-td font-medium">{r.volunteer_name}</td>
                  <td className="table-td">
                    <span className={`badge ${GROUP_BADGE[r.group_name] || 'badge-gray'}`}>{r.group_name}</span>
                  </td>
                  <td className="table-td text-gray-500">{r.time_start}–{r.time_end}</td>
